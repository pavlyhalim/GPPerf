...
class GEMMPredictor:
    ...
    def prepare_input_data(self, input_dict):
        ...
        # Ensure categorical columns are treated as string type
    ...
    def estimate_power(df):
        # Power estimation based on GPU utilization and FLOPS
        BASE_POWER = 30  # Base power consumption
        MAX_POWER = 200 # Max TDP
        MAX_TFLOPS = 40 # RTX 4070 peak TFLOPS
        ...
        # Use estimated power where actual power is missing
        df['power'] = df['power'].fillna(df['estimated_power'])
        
        return df

    def filter_power_bounds(df):
        # RTX 4070 constraints
    ...
    def impute_power(df):
        # Power typically correlates with problem size and FLOPS
        df['total_elements'] = df['m'] * df['n'] * df['k']
        
        # Create a model using rows with valid power values
...
def create_comparison_chart(current_metrics, optimal_metrics):
    ...
    current_values = [
        ...
        current_metrics['Energy'],  # Changed 'energy' to 'Energy'
        current_metrics['TFlops']   # Changed 'tflops' to 'TFlops'
    ]
    optimal_values = [
        optimal_metrics['runtime'],
        optimal_metrics['power'],
        optimal_metrics['Energy'],  # Changed 'energy' to 'Energy'
        optimal_metrics['TFlops']   # Changed 'tflops' to 'TFlops'
    ]
...
def main():
    ...
    try:
        ...
        # Add a "Calculate" button with a spinner
            ...
            with st.spinner("Analyzing performance..."):
                ...
                with tab1:
                    ...
                    with perf_col3:
                        st.metric(
                            "Energy",
                            f"{max(0.01, predictions['Energy']):.2f} J",  # Changed 'energy' to 'Energy'
                            "Total Energy"
                        )
                    
                    with perf_col4:
                        efficiency = (predictions['TFlops'] / 40) * 100  # Changed 'tflops' to 'TFlops'
                        st.metric(
                            "TFLOPS",
                            f"{predictions['TFlops']:.2f}",               # Changed 'tflops' to 'TFlops'
